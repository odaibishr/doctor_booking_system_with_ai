// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'dashboard_stats.dart';

// **************************************************************************
// TypeAdapterGenerator
// **************************************************************************

class DashboardStatsAdapter extends TypeAdapter<DashboardStats> {
  @override
  final int typeId = 15;

  @override
  DashboardStats read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return DashboardStats(
      todayAppointments: fields[0] as int,
      upcomingAppointments: fields[1] as int,
      completedAppointments: fields[2] as int,
      cancelledAppointments: fields[3] as int,
      totalPatients: fields[4] as int,
      todayPatients: fields[5] as int,
      earnings: fields[6] as EarningsData,
      reviewsAvg: fields[7] as int,
      reviewsCount: fields[8] as int,
      waitlistCount: fields[9] as int,
      hospitalName: fields[10] as String?,
      workingHours: (fields[11] as List).cast<DoctorSchedule>(),
      daysOff: (fields[12] as List)
          .map((dynamic e) => (e as Map).cast<String, dynamic>())
          .toList(),
    );
  }

  @override
  void write(BinaryWriter writer, DashboardStats obj) {
    writer
      ..writeByte(13)
      ..writeByte(0)
      ..write(obj.todayAppointments)
      ..writeByte(1)
      ..write(obj.upcomingAppointments)
      ..writeByte(2)
      ..write(obj.completedAppointments)
      ..writeByte(3)
      ..write(obj.cancelledAppointments)
      ..writeByte(4)
      ..write(obj.totalPatients)
      ..writeByte(5)
      ..write(obj.todayPatients)
      ..writeByte(6)
      ..write(obj.earnings)
      ..writeByte(7)
      ..write(obj.reviewsAvg)
      ..writeByte(8)
      ..write(obj.reviewsCount)
      ..writeByte(9)
      ..write(obj.waitlistCount)
      ..writeByte(10)
      ..write(obj.hospitalName)
      ..writeByte(11)
      ..write(obj.workingHours)
      ..writeByte(12)
      ..write(obj.daysOff);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is DashboardStatsAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}
